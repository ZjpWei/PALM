#================ PALM simulation based on MIDASim (effect size scenario) ================#
  ...
  for(Setting in c("large", "small")){
    for(tax.type in c("species", "genus")){
      ...
      s <- 1
      ## Signature effect percentage {40%}
      Ka.per <- 0.4
      ...
      seed <- 2023
      set.seed(s + seed)
      data.loc <- paste0("./Simulation/Effect_size/Sim_Ka", Ka.per, "_Pos", pos.pt, "_mu", mu, "_", s,".Rdata")
      ...
      pval.het <- NULL
      for(k in 1:nrow(AA.est)){
        nonna.id <- !is.na(AA.est[k,]) & (AA.var[k,] != 0)
        if(sum(nonna.id) > 1){
          m <- metafor::rma(yi = AA.est[k,nonna.id], vi = AA.var[k,nonna.id], method = "EE")
          pval.het <- c(pval.het, m$QEp)
        }else{
      ...
      rel.abd <- list("Pool" = Y.pool)
      covariate.interest <- list("Pool" = data.frame(disease = X.pool))
      cov.adj <- list("Pool" = data.frame(study = factor(study)))
      ...
      ## high abd FDR
      select.feature <- intersect(feature.ids[qval.sin <= target.fdr & ANCOMBC2.model$res$passed_ss_labels1], signal.most.abund)
      ep.a.fdr <- c(ep.a.fdr, length(setdiff(select.feature, signal.m.abd)) / length(select.feature))
      ep.a.power <- c(ep.a.power, length(intersect(select.feature, signal.m.abd)) / length(signal.m.abd))
      ep.a.het.fdr <- c(ep.a.het.fdr, NA)

      ## low abd FDR
      select.feature <- intersect(feature.ids[qval.sin <= target.fdr & ANCOMBC2.model$res$passed_ss_labels1], signal.less.abund)
      ep.l.fdr <- c(ep.l.fdr, length(setdiff(select.feature, signal.l.abd)) / length(select.feature))
      ep.l.power <- c(ep.l.power, length(intersect(select.feature, signal.l.abd)) / length(signal.l.abd))
      ...
      ## DESeq2
      objs <- DESeqDataSetFromMatrix(countData = feature.table,
                                     colData = meta.data,
                                     design = ~ labels + study)
      ...
      ## Calculate FDR
      qval.sin <- DESeq2.test$padj
      feature.ids <- rownames(DESeq2.test)
      select.feature <- feature.ids[which(qval.sin <= target.fdr)]
      ep.fdr <- c(ep.fdr, length(setdiff(select.feature, signal.names)) / length(select.feature))
      ep.power <- c(ep.power, length(intersect(select.feature, signal.names)) / length(signal.names))
      ep.het.fdr <- c(ep.het.fdr, NA)
      ...
      locom.model <- locom(otu.table = t(feature.table),
                           Y = as.numeric(meta.data$labels) - 1,
                           C = as.numeric(meta.data$study),
                           filter.thresh = 0,
                           n.cores = 1)

      locom.res <- data.frame(effect.size = locom.model$effect.size[1,],
                              p.value = locom.model$p.otu[1,],
                              q.value = locom.model$q.otu[1,],
                              row.names = colnames(locom.model$p.otu))

      ## Calculate FDR
      qval.sin <- locom.res$q.value
      feature.ids <- rownames(locom.res)
      select.feature <- feature.ids[which(qval.sin <= target.fdr)]
      ep.fdr <- c(ep.fdr, length(setdiff(select.feature, signal.names)) / length(select.feature))
      ep.power <- c(ep.power, length(intersect(select.feature, signal.names)) / length(signal.names))
      ep.het.fdr <- c(ep.het.fdr, NA)
