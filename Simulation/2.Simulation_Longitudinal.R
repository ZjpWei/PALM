#================ PALM simulation based on MIDASim (Longitudinal)================#
  ...
  for(Setting in c("small", "large")){
    for(tax.type in c("species", "genus")){
      ...
      seed <- 2023
      set.seed(s + seed)
      data.loc <- paste0("./Longitudinal/Sim_Ka", Ka.per, "_Pos", pos.pt, "_mu", mu, "_", s,".Rdata")
      ...
      for(l in 1:L){
        ...
        ## control data
        sample.id <- sample(1:nrow(Y), size = ceiling(n/4), replace = TRUE)
        if(!seq.grp){
        ...
        input.taxa.prop <- count.ibd.setup$taxa.1.prop * (sum(input.sample.prop) * ncol(Y) / ceiling(n/4)) / sum(count.ibd.setup$taxa.1.prop)

        ## Simulate data
        ### base
        count.ibd.modified.base <- MIDASim.modify(count.ibd.setup,
                                                  lib.size = lib.size,
                                                  mean.rel.abund = mean.avg.prop,
                                                  sample.1.prop = input.sample.prop,
                                                  taxa.1.prop = input.taxa.prop)

        simulated.data.base <- MIDASim(count.ibd.modified.base)

        control.data.base <- simulated.data.base$sim_count

        ### X1
        count.ibd.modified.X1 <- MIDASim.modify(count.ibd.setup,
                                                lib.size = lib.size,
                                                mean.rel.abund = mean.avg.prop,
                                                sample.1.prop = input.sample.prop,
                                                taxa.1.prop = input.taxa.prop)

        simulated.data.X1 <- MIDASim(count.ibd.modified.X1)

        control.data.X1 <- simulated.data.X1$sim_count

        ### X2
        count.ibd.modified.X2 <- MIDASim.modify(count.ibd.setup,
                                                lib.size = lib.size,
                                                mean.rel.abund = mean.avg.prop,
                                                sample.1.prop = input.sample.prop,
                                                taxa.1.prop = input.taxa.prop)

        simulated.data.X2 <- MIDASim(count.ibd.modified.X2)

        control.data.X2 <- simulated.data.X2$sim_count

        control.data <- NULL
        for(i in 1:ceiling(n/4)){
          control.data <- rbind(control.data, (control.data.base[i,] + control.data.X1[i,]) / 2)
          control.data <- rbind(control.data, (control.data.base[i,] + control.data.X2[i,]) / 2)
        }
        ...
        case.data.base <- simulated.data.base$sim_count
        ...
        case.data.X1 <- simulated.data.X1$sim_count
        ...
        case.data.X2 <- simulated.data.X2$sim_count

        case.data <- NULL
        for(i in 1:ceiling(n/4)){
          case.data <- rbind(case.data, (case.data.base[i,] + case.data.X1[i,]) / 2)
          case.data <- rbind(case.data, (case.data.base[i,] + case.data.X2[i,]) / 2)
        }

        ## summarize data
        tmp.data <- matrix(0, nrow = ceiling(n/4) * 4, ncol = ncol(Y.study[[l]]),
                           dimnames = list(paste0("C",l,"S", 1:(ceiling(n/4) * 4)), colnames(Y.study[[l]])))
        tmp.data[,non.filter] <- rbind(control.data, case.data)

        case.count <- tmp.data
        X.lst<- rep(c(0,1), each = ceiling(n/4) * 2)

        data.rel[[l]] <- list(Y = round(case.count)[1:n,], X = X.lst[1:n], subject = rep(1:(ceiling(n/4) * 2), each = 2)[1:n])
      }
      ...
      for(l in 1:L){
        ...
        clusters[[paste0("S",l)]] <- data.rel[[l]]$subject
        cov.adj <- c(cov.adj, paste0("S", l, "_",data.rel[[l]]$subject))
      }
      ...
      meta.data = data.frame(labels = factor(outcome), study = factor(study), subject = cov.adj)
      ...
      ## ANCOM-BC2  without sensitivity score filter
      ...
      for(l in 1:L){
        ...
        ANCOMBC2.model <- try(
          ancombc.fun(feature.table = tmp.data[rowSums(tmp.data)!=0,],
        ...
        if(class(ANCOMBC2.model) != "try-error"){
      ...
      ## Heterogeneity test for ANCOMBC2 with ss
      pval.het <- NULL
      ...
      for(l in 1:L){
        ...
        inormal <- apply(clrx_Z, 2, function(d){
          data.d <- data.frame(labels = meta.data$labels[meta.data$study == l],
                               d = d, subject = meta.data$subject[meta.data$study == l])
          clr.fit <- try(lme4::lmer(d ~ labels + (1 | subject),  data = data.d))
          if(class(clr.fit) != "try-error"){
            sum.fit <- summary(clr.fit)
      ...
      summary.score <- palm.get.summary(null.obj = null_obj, cluster = clusters, covariate.interest = covariate.interest)
      ...
      names(qval.het) <- PALM.model$disease$feature
      select.feature <- feature.ids[qval.sin <= target.fdr]
      ep.fdr <- c(ep.fdr, length(setdiff(select.feature, signal.names)) / length(select.feature))
      ep.power <- c(ep.power, length(intersect(select.feature, signal.names)) / length(signal.names))
      ep.het.fdr <- c(ep.het.fdr, mean(qval.het <= target.het.fdr, na.rm = TRUE))
