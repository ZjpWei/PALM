...
# =============================================== #
  ...
  for(l in 1:length(data.rel)){
    ...
    Study <- rep("Original data", length(data.rel[[l]]$X))
    ...
    Study <- c(Study, rep("Simulated data", length(data.rel[[l]]$X)))
    ...
    PCoA <- data.frame(PCo1 = iMDS$vectors[,"Axis.1"], PCo2 = iMDS$vectors[,"Axis.2"], Data = Study,
  ...
  ### Generate figures ----
  pdf("./Figure/Supp_PCoA.pdf", width = 10.35, height = 6.95, bg = "white")
