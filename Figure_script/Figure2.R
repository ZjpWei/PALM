...
# =============================================== #
  ...
  # Setup path.
  data.loc <- "./Simulation/Longitudinal/"
  ...
  p.large.het.species <- PRC_all %>% dplyr::filter(Settings == "large", tax.type == "species", Study == "meta") %>%
    ...
    geom_point(size = 2,aes(color = Method), pch = 18,
               position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = pmax(het - sd.het, 0), ymax = pmin(het + sd.het, 0.2), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylab("Proportion of het. features") + ylim(0, 0.2) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LM-CLR",  "MaAsLin2", "PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "red")) +
    theme_bw() +
  ...
  p.small.het.species <- PRC_all %>% dplyr::filter(Settings == "small", tax.type == "species", Study == "meta") %>%
    ...
    geom_point(size = 2,aes(color = Method), pch = 18,
               position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = pmax(het - sd.het, 0), ymax = pmin(het + sd.het, 0.2), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylim(0, 0.2) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LM-CLR",  "MaAsLin2", "PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "red")) +
    theme_bw() +
  ...
  pdf("./Figure/Figure2_FDR.pdf", width = 21, height = 5.6, bg = "white")

  pp3_1

  dev.off()

  pdf("./Figure/Figure2_Power.pdf", width = 21, height = 5.6, bg = "white")

  pp3_2

  dev.off()

  pdf("./Figure/Figure2_het.pdf", width = 21, height = 5.6, bg = "white")
