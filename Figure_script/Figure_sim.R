...
# =============================================== #
  ...
  # Scenario: n = 50 ----
  # Scenario: n = 200 ----
  data.loc <- "./Simulation/Multi_com_MIDA/"
  ...
  for(pos.lst in c(0.5, 1)){
    for(u.lst in c(0, 1)){
      for(tag in c(0.05, 0.1, 0.15, 0.2)){
        PRC_tmp <- NULL
        for(s in 1:100){
          if(file.exists(paste0(data.loc, "AUPRC_Ka", tag, "_Pos", pos.lst, "_effsz10_mu", u.lst, "_", as.character(s), ".Rdata"))){
            load(paste0(data.loc, "AUPRC_Ka", tag, "_Pos", pos.lst, "_effsz10_mu", u.lst, "_", as.character(s), ".Rdata"))
        ...
        tmp.prc <- data.frame(PRC_tmp) %>% group_by(method, Settings, tax.type, Study) %>%
          summarize(FDR = mean(ep.fdr), sd.FDR = sd(ep.fdr),
                    ...
                    het = mean(ep.fdr.1e1), sd.het = sd(ep.fdr.1e1))
        tmp.prc$x.label <- tag
        if(pos.lst == 0.5){
          tmp.prc$pos <- paste0("Balanced +/-")
        }else if(pos.lst == 1){
          tmp.prc$pos <- paste0("Dominate +")
        }
        if(u.lst == 0){
  ...
  p.large.pw.species <- PRC_all %>% dplyr::filter(Settings == "large", tax.type == "species", Study == "meta") %>%
    ...
    geom_point(size = 2,aes(color = Method), pch = 18,
               position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = pmax(Power - sd.Power, 0), ymax = pmin(Power + sd.Power, 1), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylim(0.4, 1) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LM-CLR",  "MaAsLin2", "PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "red")) +
    theme_bw() +
  ...
  p.large.het.species <- PRC_all %>% dplyr::filter(Settings == "large", tax.type == "species", Study == "meta") %>%
    ...
    geom_point(size = 2,aes(color = Method), pch = 18,
               position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = pmax(het - sd.het, 0), ymax = pmin(het + sd.het, 1), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylab("Proportion of het. taxa") + ylim(0, 0.45) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LM-CLR",  "MaAsLin2", "PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "red")) +
    theme_bw() +
  ...
  p.small.fdr.species <- PRC_all %>% dplyr::filter(Settings == "small", tax.type == "species", Study == "meta") %>%
    ...
    geom_point(size = 2,aes(color = Method), pch = 18,
               position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = FDR - sd.FDR, ymax = pmin(FDR + sd.FDR, 1), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylim(0, 1) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LM-CLR",  "MaAsLin2", "PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "red")) +
  ...
  p.small.het.species <- PRC_all %>% dplyr::filter(Settings == "small", tax.type == "species", Study == "meta") %>%
    ...
    geom_point(size = 2,aes(color = Method), pch = 18,
               position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = pmax(het - sd.het, 0), ymax = pmin(het + sd.het, 1), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylab("Proportion of het. taxa") + ylim(0, 0.455) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LM-CLR",  "MaAsLin2", "PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "red")) +
    theme_bw() +
  ...
  pp1_1 <- ggpubr::annotate_figure(
    ...
    bottom = ggpubr::text_grob("Proportion of covariate-associated taxa", size = 20, face = "bold"),
  ...
  pp1_3 <- ggpubr::annotate_figure(
    ggpubr::ggarrange(p.large.het.species, p.large.het.genus, p.small.het.species, p.small.het.genus,
                      nrow = 1, ncol = 4,  common.legend = TRUE, legend = "none", label.y = "AUPRC") ,
    bottom = ggpubr::text_grob("Proportion of covariate-associated taxa", size = 20, face = "bold"),
    left = ggpubr::text_grob("Proportion of het. taxa", size = 20, rot = 90, hjust = 0.4, face = "bold"),
  )

  pdf("./Figure/Figure_sim_FDR.pdf", width = 21, height = 5.6, bg = "white")

  pp1_1

  dev.off()

  pdf("./Figure/Figure_sim_Power.pdf", width = 21, height = 5.6, bg = "white")

  pp1_2

  dev.off()

  pdf("./Figure/Figure_sim_heterogeneity.pdf", width = 21, height = 5.6, bg = "white")
  ...
  ## Supplementary (a)
  ## Generate figures large/species
  p.large.fdr.species <- PRC_all %>% dplyr::filter(Settings == "large", tax.type == "species", (Study == "Original" | Method == "PALM")) %>%
    ggplot(aes(x=x.label, y=FDR, group = Method)) +
    geom_hline(aes(yintercept = 0.05),colour="#990000", linetype="dashed") +
    geom_line(linewidth = 0.7, aes(color=Method),
              position = position_dodge(width = 0.3)) +
  ...
  p.large.pw.species <- PRC_all %>% dplyr::filter(Settings == "large", tax.type == "species", (Study == "Original" | Method == "PALM")) %>%
    ggplot(aes(x=x.label, y=Power, group = Method)) +
    geom_line(linewidth = 0.7, aes(color=Method),
              position = position_dodge(width = 0.3)) +
    geom_point(size = 2,aes(color = Method), pch = 18,
               position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = pmax(Power - sd.Power, 0), ymax = pmin(Power + sd.Power, 1), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylim(0.35, 1) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LM-CLR",  "MaAsLin2", "PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "red")) +
    theme_bw() +
  ...
  ## Generate figures small/species
  p.small.fdr.species <- PRC_all %>% dplyr::filter(Settings == "small", tax.type == "species", (Study == "Original" | Method == "PALM")) %>%
    ggplot(aes(x=x.label, y=FDR, group = Method)) +
    geom_hline(aes(yintercept = 0.05),colour="#990000", linetype="dashed") +
    geom_line(linewidth = 0.7, aes(color=Method),
              position = position_dodge(width = 0.3)) +
  ...
  p.small.pw.species <- PRC_all %>% dplyr::filter(Settings == "small", tax.type == "species", (Study == "Original" | Method == "PALM")) %>%
    ggplot(aes(x=x.label, y=Power, group = Method)) +
    geom_line(linewidth = 0.7, aes(color=Method),
              position = position_dodge(width = 0.3)) +
    geom_point(size = 2,aes(color = Method), pch = 18,
  ...
  ## Generate figures large/genus
  p.large.fdr.genus <- PRC_all %>% dplyr::filter(Settings == "large", tax.type == "genus", (Study == "Original" | Method == "PALM")) %>%
    ggplot(aes(x=x.label, y=FDR, group = Method)) +
    geom_hline(aes(yintercept = 0.05),colour="#990000", linetype="dashed") +
    geom_line(linewidth = 0.7, aes(color=Method),
              position = position_dodge(width = 0.3)) +
  ...
  p.large.pw.genus <- PRC_all %>% dplyr::filter(Settings == "large", tax.type == "genus", (Study == "Original" | Method == "PALM")) %>%
    ggplot(aes(x=x.label, y=Power, group = Method)) +
    geom_line(linewidth = 0.7, aes(color=Method),
              position = position_dodge(width = 0.3)) +
    geom_point(size = 2,aes(color = Method), pch = 18,
  ...
  ## Generate figures small/genus
  p.small.fdr.genus <- PRC_all %>% dplyr::filter(Settings == "small", tax.type == "genus", (Study == "Original" | Method == "PALM")) %>%
    ggplot(aes(x=x.label, y=FDR, group = Method)) +
    geom_hline(aes(yintercept = 0.05),colour="#990000", linetype="dashed") +
    geom_line(linewidth = 0.7, aes(color=Method),
              position = position_dodge(width = 0.3)) +
  ...
  p.small.pw.genus <- PRC_all %>% dplyr::filter(Settings == "small", tax.type == "genus", (Study == "Original" | Method == "PALM")) %>%
    ggplot(aes(x=x.label, y=Power, group = Method)) +
    geom_line(linewidth = 0.7, aes(color=Method),
              position = position_dodge(width = 0.3)) +
    geom_point(size = 2,aes(color = Method), pch = 18,
  ...
  pdf("./Figure/Figure_supp_a_FDR.pdf", width = 21, height = 5.6, bg = "white")

  pp2_1

  dev.off()

  pdf("./Figure/Figure_supp_a_Power.pdf", width = 21, height = 5.6, bg = "white")
  ...
  # Scenario: n = 50 ----
  # Scenario: n = 200 ----
  data.loc <- "./Simulation/Multi_MIDA_lon/"
  ...
  PRC_all$method[PRC_all$method == "LM-CLR"] <- "LMM-CLR"
  PRC_all$x.label <- factor(PRC_all$x.label, levels = unique(PRC_all$x.label), ordered = TRUE)
  PRC_all$Method <- factor(PRC_all$method, levels = c("ANCOM-BC2", "LinDA", "LMM-CLR", "MaAsLin2", "PALM"), ordered = TRUE)
  ...
  p.large.fdr.species <- PRC_all %>% dplyr::filter(Settings == "large", tax.type == "species", Study == "meta") %>%
    ...
    geom_errorbar(aes(ymin = pmax(FDR - sd.FDR, 0), ymax = pmin(FDR + sd.FDR, 1), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylim(0, 1) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LMM-CLR",  "MaAsLin2", "PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "red")) +
    theme_bw() +
    theme(panel.grid.major = element_line(colour = "grey", linetype = "dotted"),
          panel.grid.minor = element_blank(),
  ...
  p.small.het.species <- PRC_all %>% dplyr::filter(Settings == "small", tax.type == "species", Study == "meta") %>%
    ...
    geom_point(size = 2,aes(color = Method), pch = 18,
               position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = pmax(het - sd.het, 0), ymax = pmin(het + sd.het, 1), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylim(0, 0.455) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LMM-CLR",  "MaAsLin2", "PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "red")) +
    theme_bw() +
  ...
  p.small.het.genus <- PRC_all %>% dplyr::filter(Settings == "small", tax.type == "genus", Study == "meta") %>%
    ...
    geom_point(size = 2,aes(color = Method), pch = 18,
               position = position_dodge(width = 0.3)) +
    geom_errorbar(aes(ymin = pmax(het - sd.het, 0), ymax = pmin(het + sd.het, 1), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylim(0, 0.45) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LMM-CLR",  "MaAsLin2", "PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "red")) +
    theme_bw() +
  ...
  pdf("./Figure/Figure_supp_b_FDR.pdf", width = 21, height = 5.6, bg = "white")

  pp3_1

  dev.off()

  pdf("./Figure/Figure_supp_b_Power.pdf", width = 21, height = 5.6, bg = "white")

  pp3_2

  dev.off()

  pdf("./Figure/Figure_supp_b_heterogeneity.pdf", width = 21, height = 5.6, bg = "white")
