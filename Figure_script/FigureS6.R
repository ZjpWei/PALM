...
# =============================================== #
  ...
  p.large.fdr.species <- PRC_all %>% dplyr::filter(Settings == "large", tax.type == "species", (Study == "Original" | Method == "PALM")) %>%
    ...
    scale_color_manual(
      breaks = c("ANCOM-BC2", "LinDA", "LM-CLR",  "MaAsLin2", "DESeq2","LOCOM","PALM"),
      values = c("blue", "skyblue", "orange", "#4dac26", "#DDA0DD","grey","red")) +
    theme_bw() +
  ...
  p.small.pw.genus <- PRC_all %>% dplyr::filter(Settings == "small", tax.type == "genus", (Study == "Original" | Method == "PALM")) %>%
    ...
    geom_errorbar(aes(ymin = pmax(Power - sd.Power, 0.5), ymax = pmin(Power + sd.Power, 1), color = Method), width = 0.3,
                  position = position_dodge(width = 0.3)) + ylim(0.5, 1) +
    scale_color_manual(
      breaks = c("ANCOM-BC2", "DESeq2","LinDA", "LM-CLR",  "MaAsLin2", "LOCOM","PALM"),
      values = c("blue",  "#DDA0DD","skyblue", "orange", "#4dac26","grey","red")) +
    theme_bw() +
    theme(panel.grid.major = element_line(colour = "grey", linetype = "dotted"),
          panel.grid.minor = element_blank(),
          plot.title = element_blank(),
  ...
  pdf("./Figure/FigureS6_FDR.pdf", width = 21, height = 5.6, bg = "white")

  pp2_1

  dev.off()

  pdf("./Figure/FigureS6_Power.pdf", width = 21, height = 5.6, bg = "white")
