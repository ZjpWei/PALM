...
# =============================================== #
  ...
  Venn.set <- list("PALM" = taxa.id,
                   "ANCOM-BC2" = ANCOMBC2.res$features[ANCOMBC2.res$het.qval <= fdr.cut & !is.na(ANCOMBC2.res$het.qval)],
                   "MaAsLin2" = Maaslin2.res$features[Maaslin2.res$qval <= fdr.cut],
                   "LM-CLR" = lmclr.res$features[lmclr.res$qval <= fdr.cut],
                   "LinDA" = Linda.res$features[Linda.res$qval <= fdr.cut])

  venn.diagram(Venn.set, filename = "./Figure/Figure_rel_a.png",
               category.names = c("" , "" , "", "", ""),
               cex = 1.7,
