...
# =============================================== #
    ...
    for(u.lst in c(0, 1)){
      for(tag in c(0.05, 0.1, 0.15, 0.2)){
        ...
        tmp.prc <- data.frame(PRC_tmp) %>% group_by(method, Settings, tax.type, Study) %>%
          summarize(FDR = mean(ep.l.fdr, na.rm = TRUE), sd.FDR = sd(ep.l.fdr, na.rm = TRUE),
                    Power = mean(ep.l.power, na.rm = TRUE), sd.Power = sd(ep.l.power, na.rm = TRUE),
                    het = mean(ep.l.het.fdr, na.rm = TRUE), sd.het = sd(ep.l.het.fdr, na.rm = TRUE))
        tmp.prc$x.label <- tag
        if(pos.lst == 0.5){
  ...
  pdf("./Figure/FigureS5_FDR.pdf", width = 21, height = 5.6, bg = "white")

  pp1_1

  dev.off()

  pdf("./Figure/FigureS5_Power.pdf", width = 21, height = 5.6, bg = "white")

  pp1_2

  dev.off()

  pdf("./Figure/FigureS5_het.pdf", width = 21, height = 5.6, bg = "white")
