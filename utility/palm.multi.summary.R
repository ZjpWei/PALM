...
palm.multi.summary <- function(null.obj,
                               covariate.interest,
                               cluster = NULL,
                               parallel.core = NULL,
                               verbose = FALSE) {
  ...
  for(d in study.ID){
    ...
    cov.nuisance <- ncol(X) - 1
    X <- cbind(X[,-ncol(X),drop=FALSE], covariate.interest[[d]][rownames(X),,drop=FALSE])
    X <- as.matrix(X)
    X.names <- colnames(X)

    ## loop for summary statistics
    ests <- matrix(NA, nrow = ncol(Y_I), ncol = ncol(covariate.interest[[d]]),
                   dimnames = list(colnames(Y_I), X.names[(cov.nuisance+1):length(X.names)]))
    covs <- array(data = NA, dim = c(ncol(Y_I), ncol(covariate.interest[[d]]), ncol(covariate.interest[[d]])),
                  dimnames = list(colnames(Y_I), X.names[(cov.nuisance+1):length(X.names)],
                                  X.names[(cov.nuisance+1):length(X.names)]))
    for(k in colnames(Y_I)){
      ...
      beta.name <- paste0(X.names[(cov.nuisance+1):ncol(X)],":", k)
      ...
      ests[k,] <- t(solve(I_beta) %*% colSums(s.i.lst)[beta.name])
      core.U <- 0
      ...
      covs[k,,] <- solve(I_beta) %*% (core.U) %*% solve(I_beta)
    }
    summary.stat.study.one <- list(est = ests, var = covs, n = nrow(Y_I))
